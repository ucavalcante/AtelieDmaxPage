// TypeScript para página de detalhes do produto com carrossel
// Vue.js 3 Composition API

interface ProductData {
    id: string;
    name: string;
    description: string;
    folder: string;
}

interface ImageDescription {
    description: string;
    details?: string[];
}

interface ImageDescriptions {
    [key: string]: ImageDescription;
}

interface ProductDetailState {
    loading: boolean;
    error: string | null;
    productData: ProductData;
    images: string[];
    imageDescriptions: ImageDescriptions;
    currentImageIndex: number;
}

interface ProductSettings {
    basePath: string;
    fallbackImage: string;
    lazyLoading: boolean;
    imageFormats: string[];
    autoDetector: any | null;
}

interface HardcodedProductData {
    [key: string]: ProductData;
}

const { createApp, ref, reactive, computed, onMounted, onUnmounted } = Vue;

const ProductDetailApp = {
    setup() {
        // Estado reativo
        const state: ProductDetailState = reactive({
            loading: true,
            error: null,
            productData: {
                id: '',
                name: '',
                description: '',
                folder: ''
            },
            images: [],
            imageDescriptions: {},
            currentImageIndex: 0
        });

        // Configurações com suporte a subpasta (/develop)
        const isDevelop = typeof window !== 'undefined' && window.location.pathname.includes('/develop');
        const prefix = isDevelop ? '/develop' : '';

        const settings: ProductSettings = {
            basePath: `${prefix}/img/products/`,
            fallbackImage: `${prefix}/img/products/placeholder.jpg`,
            lazyLoading: true,
            imageFormats: ['webp', 'jpeg', 'jpg', 'png'],
            autoDetector: null
        };

        // Lifecycle
        onMounted(async () => {
            await initializeProduct();
        });

        // Métodos principais
        const initializeProduct = async (): Promise<void> => {
            try {
                state.loading = true;
                state.error = null;

                // Extrair ID do produto da URL
                const productId = extractProductIdFromUrl();
                if (!productId) {
                    throw new Error('ID do produto não encontrado na URL');
                }

                // Carregar dados do produto
                await loadProductData(productId);
                
                // Carregar imagens
                await loadProductImages(productId);

                // Carregar descrições (se arquivo JSON existir)
                await loadImageDescriptions(productId);

            } catch (error: any) {
                console.error('Erro ao inicializar produto:', error);
                state.error = error.message || 'Erro ao carregar produto';
            } finally {
                state.loading = false;
            }
        };

        const extractProductIdFromUrl = (): string | null => {
            const path = window.location.pathname;
            
            // Tentar diferentes padrões de URL
            let match: RegExpMatchArray | null = null;
            
            // Padrão para servidor web: /details/produto.html
            match = path.match(/\/details\/([^\/]+)\.html$/);
            
            // Padrão para arquivo local: .../details/produto.html
            if (!match) {
                match = path.match(/\/([^\/]+)\.html$/);
            }
            
            const productId = match ? match[1] : null;
            return productId;
        };

        const loadProductData = async (productId: string): Promise<void> => {
            try {
                // Tentar carregar do products.json primeiro
                const isDevelop = typeof window !== 'undefined' && window.location.pathname.includes('/develop');
                const prefix = isDevelop ? '/develop' : '';
                const response = await fetch(`${prefix}/data/products.json`);
                if (response.ok) {
                    const data = await response.json();
                    const category = data.categories.find((cat: any) => cat.id === productId || cat.folder === productId);
                    
                    if (category) {
                        state.productData = {
                            id: category.id || category.folder,
                            name: category.name,
                            description: category.description,
                            folder: category.folder || category.id
                        };
                        return;
                    }
                }
                
                // Fallback para dados hardcoded
                state.productData = getHardcodedProductData(productId);
                
            } catch (error) {
                console.warn('Erro ao carregar products.json, usando dados hardcoded:', error);
                state.productData = getHardcodedProductData(productId);
            }
        };

        const getHardcodedProductData = (productId: string): ProductData => {
            const hardcodedData: HardcodedProductData = {
                'enxoval-bebe': {
                    id: 'enxoval-bebe',
                    name: 'Enxoval de Bebê',
                    description: 'Enxovais personalizados com bordados únicos para bebês',
                    folder: 'enxoval-bebe'
                },
                'sapato-infantil': {
                    id: 'sapato-infantil',
                    name: 'Sapato Infantil Customizado',
                    description: 'Sapatos infantis personalizados com bordados e decorações únicas',
                    folder: 'sapato-infantil'
                },
                'batizado': {
                    id: 'batizado',
                    name: 'Batizado',
                    description: 'Artigos especiais para cerimônias de batizado',
                    folder: 'batizado'
                },
                'caixa-convite': {
                    id: 'caixa-convite',
                    name: 'Caixa Convite',
                    description: 'Convites em caixas personalizadas para ocasiões especiais',
                    folder: 'caixa-convite'
                },
                'ocasioes-especiais': {
                    id: 'ocasioes-especiais',
                    name: 'Ocasiões Especiais',
                    description: 'Artesanatos únicos para momentos inesquecíveis',
                    folder: 'ocasioes-especiais'
                },
                'depoimentos': {
                    id: 'depoimentos',
                    name: 'Depoimentos',
                    description: 'Avaliações e comentários de nossos queridos clientes',
                    folder: 'depoimentos'
                }
            };

            return hardcodedData[productId] || {
                id: productId,
                name: productId.charAt(0).toUpperCase() + productId.slice(1),
                description: 'Produto artesanal único do Ateliê Dmax',
                folder: productId
            };
        };

        const loadProductImages = async (productId: string): Promise<void> => {
            try {
                if ((window as any).autoDetector) {
                    console.log('Usando auto-detector para carregar imagens...');
                    
                    await (window as any).autoDetector.loadCategoriesFromJson();
                    const detectedCategory = await (window as any).autoDetector.detectImagesForCategory(state.productData.folder);
                    
                    if (detectedCategory && detectedCategory.images.length > 0) {
                        state.images = detectedCategory.images;
                        console.log(`✅ ${state.images.length} imagens detectadas para ${productId}`);
                        return;
                    }
                }

                throw new Error('Nenhuma imagem encontrada para este produto via API ou Diretório');

            } catch (error: any) {
                console.error('Erro ao carregar imagens:', error);
                state.error = error.message || 'Erro ao carregar imagens do produto';
            }
        };

        const loadImageDescriptions = async (productId: string): Promise<void> => {
            try {
                const descriptionsUrl = `/data/descriptions/${productId}.json`;
                const response = await fetch(descriptionsUrl);
                
                if (response.ok) {
                    const descriptions: ImageDescriptions = await response.json();
                    state.imageDescriptions = descriptions;
                    console.log('✅ Descrições de imagens carregadas do arquivo centralizado');
                } else {
                    console.log('ℹ️ Arquivo de descrições não encontrado, usando descrições padrão');
                }
            } catch (error) {
                console.log('ℹ️ Nenhuma descrição personalizada disponível');
            }
        };

        // Navegação do carrossel
        const nextImage = (): void => {
            if (state.images.length > 1) {
                state.currentImageIndex = (state.currentImageIndex + 1) % state.images.length;
            }
        };

        const previousImage = (): void => {
            if (state.images.length > 1) {
                state.currentImageIndex = state.currentImageIndex === 0 
                    ? state.images.length - 1 
                    : state.currentImageIndex - 1;
            }
        };

        const goToImage = (index: number): void => {
            if (index >= 0 && index < state.images.length) {
                state.currentImageIndex = index;
            }
        };

        // Navegação por teclado
        const handleKeyNavigation = (event: KeyboardEvent): void => {
            switch (event.key) {
                case 'ArrowLeft':
                    event.preventDefault();
                    previousImage();
                    break;
                case 'ArrowRight':
                    event.preventDefault();
                    nextImage();
                    break;
                case 'Escape':
                    event.preventDefault();
                    goBack();
                    break;
            }
        };

        onMounted(() => {
            document.addEventListener('keydown', handleKeyNavigation);
        });

        onUnmounted(() => {
            document.removeEventListener('keydown', handleKeyNavigation);
        });

        // Utilitários
        const getCurrentImageUrl = (): string => {
            if (state.images.length === 0) return settings.fallbackImage;
            const imageName = state.images[state.currentImageIndex];
            return getImageUrl(imageName);
        };

        const getImageUrl = (imageName: string): string => {
            return `${settings.basePath}${state.productData.folder}/${imageName}`;
        };

        const getCurrentImageDescription = (): string => {
            if (state.images.length === 0) return '';
            
            const imageName = state.images[state.currentImageIndex];
            const key = imageName.replace(/\.[^/.]+$/, ''); // Remove extensão
            
            return state.imageDescriptions[key]?.description || 
                   state.imageDescriptions[imageName]?.description ||
                   `${state.productData.name} - Imagem ${state.currentImageIndex + 1}`;
        };

        const getImageDescription = (imageName: string, index: number): string => {
            const key = imageName.replace(/\.[^/.]+$/, '');
            return state.imageDescriptions[key]?.description || 
                   state.imageDescriptions[imageName]?.description ||
                   `${state.productData.name} - Imagem ${index + 1}`;
        };

        const getCurrentImageData = (): ImageDescription => {
            if (state.images.length === 0) return { description: '' };
            
            const imageName = state.images[state.currentImageIndex];
            const key = imageName.replace(/\.[^/.]+$/, '');
            
            return state.imageDescriptions[key] || state.imageDescriptions[imageName] || { description: '' };
        };

        const handleImageError = (event: Event): void => {
            const target = event.target as HTMLImageElement;
            console.warn('Erro ao carregar imagem:', target.src);
            target.src = settings.fallbackImage;
        };

        const getWhatsAppLink = (): string => {
            const productName = encodeURIComponent(state.productData.name);
            const message = encodeURIComponent(
                `Olá! Gostaria de mais informações sobre: ${state.productData.name}`
            );
            return `https://api.whatsapp.com/send?phone=551198338-7957&text=${message}`;
        };

        const goBack = (): void => {
            // Verificar se veio de uma página interna
            if (document.referrer && document.referrer.includes(window.location.origin)) {
                window.history.back();
            } else {
                // Ir para a página principal
                const isDevelop = typeof window !== 'undefined' && window.location.pathname.includes('/develop');
                const prefix = isDevelop ? '/develop/' : '/';
                window.location.href = prefix;
            }
        };

        const retryLoad = async (): Promise<void> => {
            await initializeProduct();
        };

        // Computed properties
        const hasMultipleImages = computed(() => state.images.length > 1);

        return {
            // Estado
            ...Vue.toRefs(state),
            
            // Computed
            hasMultipleImages,
            
            // Métodos
            nextImage,
            previousImage,
            goToImage,
            getCurrentImageUrl,
            getImageUrl,
            getCurrentImageDescription,
            getImageDescription,
            getCurrentImageData,
            handleImageError,
            getWhatsAppLink,
            goBack,
            retryLoad
        };
    }
};

// Inicializar aplicação quando DOM estiver pronto
document.addEventListener('DOMContentLoaded', () => {
    const app = createApp(ProductDetailApp);
    app.mount('#product-detail-app');
});
